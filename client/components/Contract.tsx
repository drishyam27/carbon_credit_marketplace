...
const PURCHASE_STATUS_CONFIG: Record<string, { color: string; bg: string; label: string }> = {
  Pending: { color: "text-[#fbbf24]", bg: "bg-[#fbbf24]/10", label: "Awaiting Confirmation" },
  Confirmed: { color: "text-[#34d399]", bg: "bg-[#34d399]/10", label: "Completed" },
  Cancelled: { color: "text-[#f87171]", bg: "bg-[#f87171]/10", label: "Cancelled" },
  Disputed: { color: "text-[#f59e0b]", bg: "bg-[#f59e0b]/10", label: "Disputed" },
  Resolved: { color: "text-[#8b5cf6]", bg: "bg-[#8b5cf6]/10", label: "Resolved" },
};
...
export default function ContractUI({ walletAddress, onConnect, isConnecting }: ContractUIProps) {
  ...
  // Browse listings
  const [listings, setListings] = useState<CreditData[]>([]);
  ...
  // My credits
  const [myCredits, setMyCredits] = useState<CreditData[]>([]);
  ...
  const getPurchaseStatusConfig = (status: string | Record<string, unknown>) => {
    const key = typeof status === 'string' ? status : (Object.keys(status || {})[0] || "Pending");
    return PURCHASE_STATUS_CONFIG[key] || PURCHASE_STATUS_CONFIG.Pending;
  };

  return (
    <div className="w-full max-w-2xl animate-fade-in-up-delayed px-0 sm:px-0">
      {/* Toasts */}
      {error && (
      ...
      <Spotlight className="rounded-2xl">
        <AnimatedCard className="p-0" containerClassName="rounded-2xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b border-white/[0.06] px-4 sm:px-6 py-3 sm:py-4">
          ...
          {/* Tabs */}
          <div className="flex border-b border-white/[0.06] px-1 sm:px-2 overflow-x-auto scrollbar-hide">
            {tabs.map((t) => (
              <button
                key={t.key}
                onClick={() => { setActiveTab(t.key); setError(null); }}
                className={cn(
                  "relative flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-3 sm:py-3.5 text-xs sm:text-sm font-medium transition-all whitespace-nowrap",
                  activeTab === t.key ? "text-white/90" : "text-white/35 hover:text-white/55"
                )}
              >
                <span style={activeTab === t.key ? { color: t.color } : undefined}>{t.icon}</span>
                <span className="hidden xs:inline">{t.label}</span>
                <span className="xs:hidden">{t.key === "my-credits" ? "Credits" : t.label.split(" ")[0]}</span>
                {activeTab === t.key && (
                  <span
                ...
                {isLoadingListings ? (
                  ...
                  <div className="space-y-4">
                    {listings.map((listing) => {
                      ...
                      const isMyListing = walletAddress && listing.owner_address === walletAddress;
                      return (
                        <div key={lid} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3">
                          <div className="flex items-center justify-between">
                            <h4 className="font-medium text-white/90 text-sm">{String(listing.project_name)}</h4>
                            <span className="text-[10px] px-2 py-1 rounded-full bg-[#34d399]/10 text-[#34d399]">
                              Active
                            </span>
                          </div>
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-white/30">Available:</span>
                            <span className="font-mono text-white/70">{formatAmount(listing.carbon_amount)} tons</span>
                          </div>
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-white/30">Price:</span>
                            <span className="font-mono text-[#34d399]">{formatAmount(listing.price_per_ton)} XLM/ton</span>
                          </div>
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-white/30">Seller:</span>
                            <span className="font-mono text-white/50">{truncate(listing.owner_address)}</span>
                          </div>
                          ...
                          {!isMyListing && walletAddress && (
                            <div className="pt-2 border-t border-white/[0.06] flex flex-col sm:flex-row gap-2">
                              <input
                                ...
                                value={buyAmount[lid] || ""}
                                onChange={(e) => setBuyAmount(prev => ({ ...prev, [lid]: e.target.value }))}
                              ...
                              />
                              <ShimmerButton 
                                onClick={() => handleBuyCredits(lid)} 
                                shimmerColor="#34d399" 
                                className="text-xs px-4 w-full sm:w-auto"
                              >
                                Buy
                              </ShimmerButton>
                            </div>
            ...
            {activeTab === "my-credits" && (
              <div className="space-y-5">
                {isLoadingCredits ? (
                  ...
                  <button onClick={onConnect} className="w-full rounded-xl border border-dashed border-[#4fc3f7]/20 bg-[#4fc3f7]/[0.03] py-4 text-sm text-[#4fc3f7]/60 hover:border-[#4fc3f7]/30 hover:text-[#4fc3f7]/80 active:scale-[0.99] transition-all">
                    Connect wallet to view credits
                  </button>
                ) : myCredits.length === 0 ? (
                  <div className="text-center py-8 text-white/30 text-sm">No credits yet. Create a listing or buy some!</div>
                ) : (
                  <div className="space-y-4">
                    {myCredits.map((credit) => (
                      <div key={String(credit.id)} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-2">
                        <h4 className="font-medium text-white/90 text-sm">{String(credit.project_name)}</h4>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-white/30">Amount:</span>
                          <span className="font-mono text-white/70">{formatAmount(credit.carbon_amount)} tons</span>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-white/30">Listed:</span>
                          <span className="font-mono text-white/70">{credit.is_listed ? "Yes" : "No"}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {!walletAddress ? (
                  ...
                  <div className="space-y-4">
                    {myPurchases.map((purchase) => {
                      const statusKey = typeof purchase.status === 'string' ? purchase.status : Object.keys(purchase.status)[0];
                      const status = getPurchaseStatusConfig(purchase.status);
                      const isBuyer = walletAddress === purchase.buyer;
                      
                      return (
                        <div key={String(purchase.id)} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4 space-y-3">
                          <div className="flex items-center justify-between">
                            <span className="text-xs text-white/30">Purchase #{String(purchase.id)} (Credit #{String(purchase.credit_id)})</span>
                          ...
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-white/30">Amount:</span>
                            <span className="font-mono text-white/70">{formatAmount(purchase.amount_purchased)} tons</span>
                          </div>
                          
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-white/30">Escrowed:</span>
                            <span className="font-mono text-[#34d399]">{formatAmount(purchase.locked_funds)} XLM</span>
                          </div>
                          
                          <div className="flex items-center justify-between text-xs">
                            <span className="text-white/30">Role:</span>
                            <span className="font-mono text-white/70">{isBuyer ? "Buyer" : "Seller"}</span>
                          </div>

                          <div className="pt-2 border-t border-white/[0.06] flex flex-col sm:flex-row gap-2">
                            {isBuyer && statusKey === "Pending" && (
                              <>
                                <ShimmerButton 
          ...
          {/* Footer */}
          <div className="border-t border-white/[0.04] px-4 sm:px-6 py-3 flex flex-col sm:flex-row items-center justify-between gap-2">
